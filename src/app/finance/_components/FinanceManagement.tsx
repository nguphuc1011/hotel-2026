...
export default function FinanceManagement() {
  ...
  const fetchData = async () => {
    try {
      ...
      const { data: ledgerData, error: ledgerError } = await supabase
        ...
        .select(
          ...
          *,
          ...
          bookings:booking_id(rooms:room_id(room_number))
        `
        )
        .eq('status', 'completed')
      ...
      const mappedTransactions = (ledgerData || []).map((l) => {
        let content = l.description || '';

        // Nếu có booking_id và room_number, thay thế UUID trong description bằng số phòng
        const roomNumber = l.bookings?.rooms?.room_number;
        if (l.booking_id && roomNumber) {
          content = content.replace(l.booking_id, `P${roomNumber}`);
        }
  ...
  useEffect(() => {
    ...
    const catChannel = supabase
      ...
      .on('postgres_changes', { event: '*', table: 'cashflow_categories', schema: 'public' }, () =>
        fetchData()
      )
      .subscribe();
  ...
  const filteredTransactions = useMemo(() => {
    ...
    return transactions.filter((t) => {
      ...
      // Type matches
      const matchesType =
        (t.type === 'income' && showIncome) || (t.type === 'expense' && showExpense);
  ...
  return (
    <div className="flex flex-col h-full bg-[#f8fafc] overflow-hidden">
        ...
        <main className="p-4 md:p-8 space-y-8 md:space-y-12 max-w-[1600px] mx-auto w-full">
          ...
          <div className="flex flex-col gap-6">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                <h2 className="text-xl md:text-2xl font-black text-[#1e293b] uppercase tracking-tighter">
                  LỊCH SỬ GIAO DỊCH
                </h2>
                <button
                  ...
                  className={cn(
                    ...
                    isFilterVisible
                      ...
                      : 'bg-white text-slate-400 shadow-[0_4px_15px_-3px_rgba(0,0,0,0.05)]'
                  )}
                >
      ...
      {/* FAB: Create New Transaction - Real Floating UI */}
      <button
        onClick={() => {
          setEditingTransaction(null);
          setIsModalOpen(true);
        }}
        className="fixed bottom-32 right-6 z-[110] w-14 h-14 bg-indigo-600 rounded-2xl flex items-center justify-center shadow-2xl shadow-indigo-300 hover:bg-indigo-700 hover:scale-110 active:scale-95 transition-all group"
      >
        <Plus
