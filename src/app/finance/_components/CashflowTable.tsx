...
export const CashflowTable: React.FC<CashflowTableProps> = ({ transactions, onEdit, onDelete }) => {
  const abbreviateContent = (content: string) => {
    ...
    // Viết tắt Phòng -> P
    abbreviated = abbreviated.replace(/Phòng\s*(\d+)/gi, 'P$1');
  ...
  const getMethodStyle = (method: string) => {
    switch (method) {
      case 'cash': return "bg-amber-100 text-amber-700 border-amber-200";
  ...
  if (transactions.length === 0) {
    return (
      <div className="bg-slate-50/50 rounded-[2rem] border border-dashed border-slate-200 p-12 flex flex-col items-center justify-center text-center">
        ...
        <h3 className="text-sm font-black text-slate-800 uppercase tracking-tight mb-1">Chưa có giao dịch</h3>
  ...
  return (
    <div className="flex flex-col gap-3">
      {transactions.map((t) => (
        <div 
          key={t.id} 
          className="group relative bg-white p-4 rounded-3xl border border-slate-100 shadow-sm hover:shadow-xl hover:border-indigo-100 transition-all duration-300 active:scale-[0.98] cursor-pointer"
          onClick={() => onEdit?.(t)}
        >
          <div className="flex items-center justify-between gap-4">
            <div className="relative flex items-center justify-between gap-3 flex-1">
              {/* 1. Creative Side-Badge Time Capsule */}
              <div className="relative flex w-[58px] h-[44px] rounded-md shadow-sm overflow-hidden shrink-0 bg-slate-100 border border-slate-200">
                {/* Left Side: Color Indicator + Arrow (Side-Badge) */}
                <div className={cn(
                  "w-[16px] flex items-center justify-center shrink-0",
                ...
                <div className="flex-1 flex flex-col">
                  ...
                  <div className="h-1/2 flex items-center justify-center bg-slate-200/30">
                    <span className="font-black text-slate-800 text-[14px] leading-none tracking-tighter">
                  ...
                  <div className="flex-1 flex items-center justify-center">
                    <span className="font-bold text-slate-600 text-[10px] tracking-tighter leading-none uppercase">
              ...
              <div className="flex flex-col min-w-0 flex-1">
                <span className="text-[13px] font-black text-slate-800 leading-tight uppercase line-clamp-2">
