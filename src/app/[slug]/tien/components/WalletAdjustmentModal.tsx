...
export default function WalletAdjustmentModal({ isOpen, onClose, onSuccess, wallets }: WalletAdjustmentModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const actionKey = diff > 0 ? 'finance_create_income' : 'finance_manual_cash_out';
    verify(actionKey as any, async (staffId?: string, staffName?: string) => {
      setLoading(true);
      try {
        const verified = staffId
          ? { id: staffId, name: staffName || '' }
          : (user ? { id: user.id, name: user.full_name || user.username || 'Unknown' } as any : undefined);
        ...
        console.error('WalletAdjustmentModal Error:', JSON.stringify(error, Object.getOwnPropertyNames(error), 2));
        toast.error(error?.message || error?.error_description || 'Có lỗi xảy ra khi điều chỉnh số dư');
      } finally {
        setLoading(false);
      }
    });
