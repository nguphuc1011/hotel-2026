CREATE OR REPLACE FUNCTION public.import_inventory(p_service_id uuid, p_qty_buy numeric, p_total_amount numeric, p_payment_method_code text DEFAULT NULL::text, p_shift_id uuid DEFAULT NULL::uuid, p_notes text DEFAULT NULL::text)
 RETURNS json
 LANGUAGE plpgsql
AS $function$
DECLARE
  v_staff_id uuid; -- Can be null
  ...
  v_service_name text;
BEGIN
  -- 1. Get Auth Info (OPTIONAL now)
  -- If no auth system, this returns null, which is fine for now.
  v_staff_id := auth.uid();
  ...
  WHERE id = p_service_id
    -- REMOVED: AND deleted_at IS NULL (Column does not exist)
    AND COALESCE(is_active, true) = true
  FOR UPDATE;

  IF NOT FOUND THEN
    RAISE EXCEPTION 'Dịch vụ không tồn tại hoặc không active';
  END IF;
  ...
  -- Keeping the fix for both columns to ensure UI works
