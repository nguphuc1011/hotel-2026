-- SQL FIX: CẬP NHẬT HÀM TẠO THANG GIÁ CHÍNH XÁC (FIX MILESTONES)
-- Mục đích: Đảm bảo các mốc nhảy tiền (Milestones) tính đúng cả giây đầu tiên sau khi hết hạn ân hạn.
...
BEGIN
    ...
    -- Mốc 0: Thời điểm hiện tại (để luôn có giá ngay lúc xem)
    v_milestones := array_append(v_milestones, v_now);

    -- A. LOGIC PHÒNG GIỜ (HOURLY)
    IF v_rental_type = 'hourly' THEN
        -- Mốc nhảy block đầu: Check-in + (base_limit * unit) + grace + 1 PHÚT
        ...
        -- QUAN TRỌNG: Phải + 1 phút để vượt qua ranh giới ân hạn (Grace Boundary)
        v_milestones := array_append(v_milestones, v_m + interval '1 minute');
    ...
    -- 3. TỔNG HỢP & TÍNH TIỀN CHÍNH XÁC
    ...
    SELECT jsonb_agg(ladder_item) INTO v_ladder FROM (
        ...
        WHERE m_time >= v_now - interval '30 minutes' -- Giữ lại mốc gần đây nhất để FE filter được
        ORDER BY m_time ASC
        LIMIT 50
    ) t;
