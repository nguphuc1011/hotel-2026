-- Update resolve_shift_variance to match "Grand Secret Order" requirements
-- 1. Two modes: ADJUST (Hạch toán) and MATCHED (Đã khớp)
-- 2. ADJUST must backdate transaction to shift.end_time (Hồi tố)

CREATE OR REPLACE FUNCTION public.resolve_shift_variance(
    p_shift_id uuid,
    p_resolution_type text, -- 'ADJUST', 'MATCHED'
    p_notes text,
    p_admin_id uuid
)
RETURNS jsonb
...
BEGIN
    ...
    -- Ensure Categories exist (Auto-create if missing to be safe, though implemented previously)
    IF v_cat_loss IS NULL THEN
        INSERT INTO public.cash_flow_categories (name, type, description, is_system)
        VALUES ('Xử lý thâm hụt ngân khố', 'OUT', 'Dùng để hạch toán khoản thiếu hụt từ ca làm việc', true)
        RETURNING id INTO v_cat_loss;
    END IF;
    IF v_cat_gain IS NULL THEN
        INSERT INTO public.cash_flow_categories (name, type, description, is_system)
        VALUES ('Xử lý thặng dư ngân khố', 'IN', 'Dùng để hạch toán khoản dư thừa từ ca làm việc', true)
        RETURNING id INTO v_cat_gain;
    END IF;
    ...
    ELSIF p_resolution_type = 'ADJUST' THEN
        ...
        IF v_variance < 0 THEN -- Missing Money (Deficit) -> Create Expense (OUT) to explain the missing amount
             INSERT INTO public.cash_flow (
                ...
                ABS(v_variance), 'OUT', v_cat_loss, '[Hạch toán ca] Xử lý thâm hụt: ' || p_notes, 'cash', p_admin_id, v_shift.end_time, 'SHIFT_ADJUSTMENT'
             );
        ELSIF v_variance > 0 THEN -- Extra Money (Surplus) -> Create Income (IN) to explain the extra amount
             INSERT INTO public.cash_flow (
                amount, flow_type, category_id, description, payment_method_code, created_by, occurred_at, transaction_type
             ) VALUES (
                v_variance, 'IN', v_cat_gain, '[Hạch toán ca] Xử lý thặng dư: ' || p_notes, 'cash', p_admin_id, v_shift.end_time, 'SHIFT_ADJUSTMENT'
             );
        END IF;
