-- MIGRATION: CHUYỂN ĐỔI SANG MULTI-TENANT (SAAS) TRIỆT ĐỂ - PHIÊN BẢN V5
...
BEGIN 
    ...
    -- Tạm thời vô hiệu hóa trigger người dùng
    FOR t_name IN 
        SELECT table_name FROM information_schema.tables 
        WHERE table_schema = 'public' AND table_type = 'BASE TABLE'
        AND table_name IN ('rooms', 'room_categories', 'bookings', 'booking_services', 'customers', 'cash_flow', 'wallets', 'external_payables', 'staff', 'role_permissions', 'services', 'cash_flow_categories', 'settings', 'audit_logs', 'inventory_logs')
    LOOP 
    ...
    UPDATE public.audit_logs SET hotel_id = v_default_hotel_id WHERE hotel_id IS NULL;
    UPDATE public.inventory_logs SET hotel_id = v_default_hotel_id WHERE hotel_id IS NULL;
...
-- A. Wallets
ALTER TABLE public.wallets DROP CONSTRAINT IF EXISTS wallets_pkey CASCADE;
...
-- C. Cash Flow Categories (Fix: Name + Type + Hotel)
ALTER TABLE public.cash_flow_categories DROP CONSTRAINT IF EXISTS cash_flow_categories_name_key CASCADE;
ALTER TABLE public.cash_flow_categories DROP CONSTRAINT IF EXISTS cash_flow_categories_type_key CASCADE;
ALTER TABLE public.cash_flow_categories DROP CONSTRAINT IF EXISTS cash_flow_categories_name_hotel_unique CASCADE;
ALTER TABLE public.cash_flow_categories ADD CONSTRAINT cash_flow_categories_unique UNIQUE (name, type, hotel_id);

-- D. Rooms
ALTER TABLE public.rooms DROP CONSTRAINT IF EXISTS rooms_room_number_key CASCADE;
ALTER TABLE public.rooms DROP CONSTRAINT IF EXISTS rooms_number_hotel_unique CASCADE;
ALTER TABLE public.rooms ADD CONSTRAINT rooms_number_hotel_unique UNIQUE (room_number, hotel_id);

-- E. Room Categories
ALTER TABLE public.room_categories DROP CONSTRAINT IF EXISTS room_categories_name_key CASCADE;
ALTER TABLE public.room_categories DROP CONSTRAINT IF EXISTS room_categories_name_hotel_unique CASCADE;
ALTER TABLE public.room_categories ADD CONSTRAINT room_categories_name_hotel_unique UNIQUE (name, hotel_id);

-- F. Services
ALTER TABLE public.services DROP CONSTRAINT IF EXISTS services_name_key CASCADE;
ALTER TABLE public.services DROP CONSTRAINT IF EXISTS services_name_hotel_unique CASCADE;
ALTER TABLE public.services ADD CONSTRAINT services_name_hotel_unique UNIQUE (name, hotel_id);

-- G. Staff
ALTER TABLE public.staff DROP CONSTRAINT IF EXISTS staff_username_key CASCADE;
ALTER TABLE public.staff DROP CONSTRAINT IF EXISTS staff_username_hotel_unique CASCADE;
ALTER TABLE public.staff ADD CONSTRAINT staff_username_hotel_unique UNIQUE (username, hotel_id);

-- H. Customers
ALTER TABLE public.customers DROP CONSTRAINT IF EXISTS customers_id_card_key CASCADE;
ALTER TABLE public.customers DROP CONSTRAINT IF EXISTS customers_id_card_hotel_unique CASCADE;
ALTER TABLE public.customers ADD CONSTRAINT customers_id_card_hotel_unique UNIQUE (id_card, hotel_id);

-- 3. CẬP NHẬT HÀM ONBOARDING
