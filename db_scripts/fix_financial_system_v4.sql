-- FIX: FINANCIAL SYSTEM V4 (CORRECT DEBT LOGIC)
-- Run this script in Supabase SQL Editor.
-- FIXES:
-- 1. Check-in now IMMEDIATELY records Debt (Receivable).
-- 2. Check-out calculates difference (Total - Already Charged).
-- 3. Ensures 'room_number' is used instead of 'name'.
...
BEGIN
    ...
    IF NEW.payment_method_code = 'credit' THEN
        -- IN increases Debt, OUT decreases Debt (Correction)
...
BEGIN
    ...
    -- 1. ADJUST DEBT (So sánh với Tiền phòng đã ghi lúc Check-in)
    ...
    IF v_diff > 0 THEN
        ...
        INSERT INTO public.cash_flow (
            ...
            'Phụ thu phòng (Chênh lệch) ' || COALESCE(v_room_name, ''), 
            now(), v_staff_id, v_staff_id, p_booking_id, true, 'credit'
        );
    ELSIF v_diff < 0 THEN
        -- Giảm nợ (Do tính thừa lúc check-in)
        INSERT INTO public.cash_flow (
            flow_type, category, amount, description, occurred_at, 
            created_by, verified_by_staff_id, ref_id, is_auto, payment_method_code
        ) VALUES (
            'OUT', 'Điều chỉnh', ABS(v_diff), 
            'Giảm trừ tiền phòng ' || COALESCE(v_room_name, ''), 
            now(), v_staff_id, v_staff_id, p_booking_id, true, 'credit'
        );
    END IF;
