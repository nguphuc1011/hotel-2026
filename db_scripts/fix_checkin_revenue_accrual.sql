-- Cập nhật hàm check_in_customer theo "Kế toán thành tích" (Accrual Basis) cho Sổ doanh thu.
-- Check-in: Ghi nhận cả Nợ (Receivable) và Doanh thu (Revenue) ngay lập tức.
...
BEGIN
    ...
    -- 3. INITIAL ROOM CHARGE (Ghi nhận nợ tức thời + Ghi nhận Doanh thu thành tích)
    DECLARE
        v_room_cat_id uuid;
    ...
    BEGIN
        ...
        IF v_initial_amount > 0 THEN
            ...
            -- Ghi nhận song song: Tăng Nợ (RECEIVABLE) và Tăng Doanh thu (REVENUE)
    ...
    -- 4. Record Cash Flow for Deposit
    IF p_deposit > 0 THEN
        INSERT INTO public.cash_flow (
            flow_type, 
            category, 
        ...
        -- Simulation for Deposit (CASH/BANK + ESCROW)
