-- MIGRATION: CHUYỂN ĐỔI SANG MULTI-TENANT (SAAS) TRIỆT ĐỂ - PHIÊN BẢN V4 (DISABLE USER TRIGGERS)
...
BEGIN 
    ...
    -- Tạm thời vô hiệu hóa tất cả trigger người dùng để cập nhật dữ liệu lịch sử
    FOR t_name IN 
        SELECT table_name FROM information_schema.tables 
        WHERE table_schema = 'public' AND table_type = 'BASE TABLE'
        AND table_name IN ('rooms', 'room_categories', 'bookings', 'booking_services', 'customers', 'cash_flow', 'wallets', 'external_payables', 'staff', 'role_permissions', 'services', 'cash_flow_categories', 'settings', 'audit_logs')
    LOOP 
