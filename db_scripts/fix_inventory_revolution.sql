-- "Cách mạng Kho hàng": Hợp nhất logic kho & Chuẩn hóa đơn vị tính
-- Căn cứ: Điều 3 (Ưu tiên thực địa Supabase) & Điều 4 (Nhất thống Mật lệnh)
-- Tác giả: Trae (AI Assistant)

-- 1. Xóa bỏ cột 'stock' dư thừa, chỉ giữ lại 'stock_quantity' là chuẩn duy nhất
-- Trước khi xóa, sync dữ liệu để đảm bảo an toàn (lấy max để tránh mất dữ liệu)
...
-- 2. Đảm bảo các cột quan trọng không bị NULL
ALTER TABLE public.services 
...
-- 3. Cập nhật hàm nhập kho (import_inventory) chuẩn hóa (Phiên bản duy nhất)
-- Logic: 
--  - Input: Số lượng mua (theo đơn vị mua, VD: Thùng), Giá tổng
--  - Process: 
--      + Quy đổi ra đơn vị bán (VD: 1 Thùng * 24 = 24 Chai)
--      + Tính lại giá vốn bình quân gia quyền (Weighted Average Cost)
--      + Tạo Transaction & CashFlow & InventoryLog
...
-- 4. Cập nhật hàm điều chỉnh kho (adjust_inventory) để đồng bộ
...
BEGIN
  ...
  -- Get old stock
  SELECT stock_quantity INTO v_old_stock FROM public.services WHERE id = p_service_id;
  
  IF NOT FOUND THEN RAISE EXCEPTION 'Service not found'; END IF;

  v_diff := p_new_quantity - v_old_stock;
  ...
  RETURN json_build_object('success', true, 'new_stock', p_new_quantity);
END;
$function$;
