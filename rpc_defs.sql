...
CREATE OR REPLACE FUNCTION public.adjust_customer_balance(p_customer_id uuid, p_amount numeric, p_type text, p_description text, p_booking_id uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
AS $function$
DECLARE
...
--- fn_create_cash_flow ---
CREATE OR REPLACE FUNCTION public.fn_create_cash_flow(p_flow_type text, p_category text, p_amount numeric, p_description text, p_occurred_at timestamp with time zone DEFAULT now())
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
...
BEGIN
    ...
    VALUES (p_flow_type, p_category, p_amount, p_description, p_occurred_at, auth.uid())
