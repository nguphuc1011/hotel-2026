...
BEGIN
    ...
    IF p_rental_type = 'hourly' THEN
        ...
        v_first_block_min := v_base_block_count * 60;
    ...
    ELSIF p_rental_type = 'overnight' THEN
        IF v_overnight_enabled = false THEN
             v_total_charge := v_price_overnight;
             v_explanation := format('Giá qua đêm (Đang tắt): %s. ', v_price_overnight);
        ELSE
             v_total_charge := v_price_overnight;
             v_explanation := format('Giá qua đêm cố định: %s. ', v_price_overnight);
        END IF;
    ...
    ELSE -- daily
        DECLARE
            v_days int;
        BEGIN
            v_days := CEIL(v_duration_minutes / 1440);
...
BEGIN
    ...
    INTO v_price_daily, v_hourly_surcharge_rate, v_auto_surcharge_enabled
    FROM public.room_categories WHERE id = p_room_cat_id;
            ...
            ELSE
                ...
                IF v_late_rules IS NOT NULL AND jsonb_array_length(v_late_rules) > 0 THEN
                     ...
                     LOOP
                        ...
                        -- Check if Checkout Time is within this rule's range (e.g., 12:00 to 15:00)
                        -- Assuming rules are ordered and non-overlapping for simplicity
                        -- Logic: If checkout time <= rule_to, use this rule.
                        -- But must be > rule_from? Yes, or just pick first match?
                        -- Better: If checkout_time <= rule_to then this rule applies.
                        -- Since we iterate in order (usually), the first match that covers the time works?
                        -- Wait, if checkout is 14:00. Rules: 12-15 (30%), 15-18 (50%).
                        -- 14:00 <= 15:00 -> Matched.
                     ...
                     -- If no rule matched (meaning time > all rule 'to' times), assume full day or max charge?
                     IF NOT v_rule_matched THEN
                         v_late_amount := v_price_daily; -- Fallback to 1 day if very late
                         v_explanation := v_explanation || format('Trả muộn quá khung giờ -> Tính 1 ngày: %s. ', v_price_daily);
                     END IF;
                ELSE
                     -- Fallback to old hardcoded if no rules
                     v_late_amount := v_price_daily * 0.3; -- default
    ...
    IF p_rental_type = 'daily' THEN
        ...
        IF (v_check_in_time < v_std_check_in) THEN
            ...
            ELSE
                 -- Dynamic Early Rules
                 -- Rules: [{"from": "05:00", "to": "09:00", "percent": 50}, {"from": "09:00", "to": "14:00", "percent": 30}]
                 -- Logic: If checkin time >= rule_from AND checkin_time < rule_to?
                 -- Actually, early checkin is reverse. 
                 -- If I check in at 06:00. 
                 -- Rule 05-09: 06:00 is inside. Use 50%.
                 -- Rule 09-14: 06:00 is NOT inside.
                 ...
                 IF v_early_rules IS NOT NULL AND jsonb_array_length(v_early_rules) > 0 THEN
                     ...
                     LOOP
                        ...
                        -- Check if checkin time is inside this window
                     ...
                     IF NOT v_rule_matched THEN
                         v_early_amount := v_price_daily * 0.5; -- Default fallback
...
BEGIN
    ...
    -- Note: Even if global is disabled, if user manually entered extra people, should we charge?
    -- User said: "lễ tân chỉ cần bấm +số người... là đc".
    -- Logic: If extra_adults > 0, we charge.
    -- But we respect the Price configuration.
...
BEGIN
    ...
    -- 4. Calc Extra Person (Manual Mode)
