-- MIGRATION: FIX HANDLE_CHECKOUT TO REMOVE TRANSACTIONS DEPENDENCY
-- Date: 2026-01-05
-- Description: Update handle_checkout to stop querying 'transactions' table which is being dropped.
--              This ensures the function works after the "CỨU HỘ" table cleanup.
...
BEGIN
    ...
    IF v_booking.customer_id IS NOT NULL THEN
        ...
        IF v_amount_paid > 0 THEN
            INSERT INTO public.ledger(customer_id, booking_id, amount, type, category, description, payment_method)
            VALUES (v_booking.customer_id, p_booking_id, v_amount_paid, 'PAYMENT', 'ROOM_BILL', 'Thanh toán trả phòng ' || p_booking_id, p_payment_method);
        END IF;
        
        -- F. Log Revenue to Ledger (Revenue is the New Charges)
        IF v_new_charges > 0 THEN
             INSERT INTO public.ledger(customer_id, booking_id, amount, type, category, description)
             VALUES (v_booking.customer_id, p_booking_id, v_new_charges, 'REVENUE', 'ROOM', 'Doanh thu phòng ' || p_booking_id);
        END IF;
