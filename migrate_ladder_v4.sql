...
BEGIN
    ...
    SELECT 
        COALESCE(price_daily, 0), surcharge_mode, surcharge_rules,
    ...
    IF v_rental_type = 'hourly' THEN
        ...
        -- Các mốc nhảy block giờ: Check-in + Base + (N * Unit) + Grace
        ...
        -- Dò tiếp 24 block tiếp theo (đảm bảo phủ hết 24h)
    ...
    IF v_rental_type = 'overnight' THEN
        v_test_time := ((v_now AT TIME ZONE v_tz)::date + v_overnight_checkout_time) AT TIME ZONE v_tz;
        IF v_test_time < v_now THEN v_test_time := v_test_time + interval '1 day'; END IF;

        -- Mốc hết giờ qua đêm + ân hạn
        v_m := v_test_time + (CASE WHEN v_grace_out_enabled THEN v_grace_minutes ELSE 0 END * interval '1 minute') + interval '1 second';
        IF v_m > v_now THEN v_milestones := array_append(v_milestones, v_m); END IF;

        -- Nếu dùng luật phụ thu (Rules)
        IF v_surcharge_mode = 'rules' AND v_surcharge_rules IS NOT NULL THEN
            FOR v_rule IN SELECT * FROM jsonb_array_elements(v_surcharge_rules) LOOP
                IF v_rule->>'type' = 'Late' THEN
                    v_m := v_test_time + ( (v_rule->>'to_minute')::int * interval '1 minute' ) + interval '1 second';
                    IF v_m > v_now THEN v_milestones := array_append(v_milestones, v_m); END IF;
                END IF;
            END LOOP;
        -- Nếu dùng tiền giờ phụ thu
        ELSIF v_surcharge_mode = 'amount' THEN
            FOR v_i IN 1..12 LOOP
                v_m := v_test_time + (v_i * interval '1 hour') + interval '1 second';
                IF v_m > v_now THEN v_milestones := array_append(v_milestones, v_m); END IF;
            END LOOP;
        END IF;
        
        -- Mốc các ngày tiếp theo
        FOR v_i IN 1..2 LOOP
            v_m := v_test_time + (v_i * interval '1 day');
            IF v_m > v_now THEN v_milestones := array_append(v_milestones, v_m); END IF;
        END LOOP;
    END IF;
    ...
    IF v_rental_type = 'daily' THEN
        ...
        -- Mốc hết giờ ngày + ân hạn
        v_m := v_test_time + (CASE WHEN v_grace_out_enabled THEN v_grace_minutes ELSE 0 END * interval '1 minute') + interval '1 second';
        IF v_m > v_now THEN v_milestones := array_append(v_milestones, v_m); END IF;

        -- Mốc nhảy sang Nguyên ngày muộn (Full day late after)
    ...
    -- Lọc trùng, lọc quá khứ, sắp xếp tăng dần
    SELECT jsonb_agg(ladder_item) INTO v_ladder FROM (
        ...
        ORDER BY m_time ASC
        LIMIT 40 -- Giới hạn để tránh mảng quá lớn
    ) t;
